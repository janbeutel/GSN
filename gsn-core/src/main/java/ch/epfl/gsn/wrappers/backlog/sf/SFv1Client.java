...
public class SFv1Client extends SFProtocol implements Runnable, BackLogMessageListener, Messenger {
	...
	private final transient Logger logger = LoggerFactory.getLogger( SFv1Client.class );
    ...
    private void readPackets() throws IOException {
		for (;;) {
			...
			try {
				...
				msg = new BackLogMessage(BackLogMessage.TOS1x_MESSAGE_TYPE, 0, data);
	...
	public boolean messageRecv(int deviceId, BackLogMessage message) {
		try {
		    if(writePacket((byte[]) message.getPayload()[0])) {
				if (logger.isDebugEnabled())
					logger.debug("Message with timestamp " + message.getTimestamp() + " successfully written to sfv1 client " + socket.getInetAddress().getHostName());
		    }
			else
		    	logger.error("Message with timestamp " + message.getTimestamp() + " could not be written to sfv1 client " + socket.getInetAddress().getHostName());
		}
		catch (IOException e) {
		    shutdown();
		}
